...
var TileObject = me.ObjectEntity.extend({
    ...
    init: function(x, y, settings) {
        if (this.type != 0) settings.image = this.type;
        if (!this.totalSize) this.totalSize = [this.size[0], this.size[1]];
    ...
    x: function(x) { //sets or gets the column at which it is located
        ...
        if (!this.hidden()) this.pos.x = (x - this.cannonTile[0]) * TILE_SIZE;
        this._x = x;
    ...
    y: function(y) { //sets or gets the row
        ...
        if (!this.hidden()) this.pos.y = (y - this.cannonTile[1]) * TILE_SIZE;
        this._y = y;
        this.onPositionChange();
        return this;
    },
...
var ItemObject = TileObject.extend({
    ...
    */
    ...
    whenOnShip: function() {
        var anim;
        if (this.rotated()) anim = this.onShipAnimations[1] !== undefined ? this.onShipAnimations[1] : this.onShipAnimations[0];
        else {
            anim = this.onShipAnimations[0];
        }
        if (anim) this.setCurrentAnimation(anim);
    },
    whenOffShip: function() {
        var anim;
        if (this.rotated()) anim = this.offShipAnimations[1] !== undefined ? this.offShipAnimations[1] : this.offShipAnimations[0];
        else {
