...
function Ship() {
    ...
    this.remove = function(item, updateBuildings) {
        if (!item) return;
        if (updateBuildings === undefined) updateBuildings = true; //updates by default
    ...
    this.mapAt = function(x, y) {
        if (ship.map()[y] !== undefined && ship.map()[y][x] !== undefined) return ship.map()[y][x];
        return null;
    };
    this.buildingsMap = {
        changed: true,
    ...
    this._getJointMap = function() {
        ...
        utils.levelTiles(function(x, y) {
            ...
            if (self.buildingsMap.get()[y][x] != charMap.codes._cleared) joint[y][x] = self.buildingsMap.get()[y][x];
        });
        return joint;
    };
...
function UserInterface() {
    ...
    this.moveGhost = function(x, y) {
        ...
        if (!this.chosen.rotated() && this.chosen.canBuildRotated(x, y)) this.chosen.rotated(true);
        if (this.chosen.rotated() && this.chosen.canBuildAt(x, y)) this.chosen.rotated(false);
        this.updateRed();
    };
    //Dragging
    this.dragging = null;
    ...
    this.mapAt = function(x, y) {
        for (var i = 0; i < this.drawingScreen.length; i++) {
            if (this.drawingScreen[i].occupies(x, y)) return this.drawingScreen[i];
        }
        var shipTile = null;
        if (ship.map()[y] !== undefined && ship.map()[y][x] !== undefined) shipTile = ship.map()[y][x];

        if (shipTile == charMap.codes._cleared && this.chosen && this.chosen.occupies(x, y)) return this.chosen;
        return shipTile;
    };
    this.init();
}